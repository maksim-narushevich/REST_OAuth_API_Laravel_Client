...
<div class="col-sm-12 col-xs-12">
    ...
    <div class="col-sm-6  col-xs-12" id="links">
        ...
        <div class="form-group" id="deleted_item" style="display: none;">User and all linked books & movies were successfully deleted!</div>
...
<script>
    ...
    $("#try_again").click(function () {
        $('#register_form').show();
        $('#request_value').html("");
        $('#block_try_again,#deleted_item').css('display', 'none');
    });
    $("#execute").click(function () {
        var intId=$('#id').val();
        var strToken=$('#token').val();
        $('#request_value').html("");
        $('#token_error,#id_error').hide();
        ...
        if (blnConfirm == true) {
            ...
            if(ajaxStatus) {
                $.ajax({
                    method: 'POST',
                    url: '{{strSubfolderRoute}}/delete_item_user_ajax',
                    data: {
                        ajaxUrlType: 'users',
                        intId: intId,
                        strToken: strToken
